...
import { t as Xe } from "./_method-ILd9Zf-V.js";
import { a as Ze, i as Qe, t as $e } from "./StatusText-DMkg47Ob.js";
import { useCallback as p, useContext as et, useEffect as tt, useMemo as m, useRef as nt, useState as rt } from "react";
import { jsx as h, jsxs as it } from "react/jsx-runtime";
//#region src/routes/pts/$method.tsx?tsr-split=component
var at = "pl-[0.9rem] pr-1.5 @sm:pl-[1.1rem] @sm:pr-3 @md:pl-[1.2rem] group-data-[stuck=true]:pb-0.5", ot = "pt-2.5 @md:pt-3 [@container_(max-height:620px)]:pt-2.5 [@container_(max-height:520px)]:pt-2.25", st = (e, t) => e?.value ? e.feeType === "percentage" ? `${e.value}%` : t(e.value) || e.value : "", ct = (e) => {
	if (!e?.value) return !1;
	let t = Number.parseFloat(e.value);
	return Number.isNaN(t) ? !0 : t !== 0;
}, lt = (e) => typeof e == "string" ? e : e.baseName;
function g() {
	let { paymentTypes: e } = Xe.useLoaderData();
	return /* @__PURE__ */ h(_, { loaderPaymentTypes: e });
}
function _({ loaderPaymentTypes: Xe = [] }) {
	let g = ye(), { resolveTextKey: _, resolveTextKeys: ut, translateKey: v } = le(), y = u(), { uiComboView: b, lockAmount: dt, uiComboView__PaymentTypes: ft, uiInteractivePrompts: pt, uiPaymentConfirmView: mt, uiBonusesStyle: x, uiShowFees: S, uiListStyle: ht, selectedPaymentType: C, initialAmount: w, method: T, currency: E, user: D, locale: gt, uiAccountDelete: _t, uiListSelectable: vt, uiFixedControls: O, uiPreselectedPaymentType: yt, onPaymentTypeSelected: k, onPaymentTypeDeselected: bt, paymentFormDraft: xt, comboViewFormDraft: St, resetFlowOnBack: Ct, setCashierState: A } = et(he), { currencyFormatter: wt, formatCurrencyValue: Tt, formatDisplayValue: Et } = xe(), { paymentTypesList: Dt, isError: Ot } = ve(), { getBonusCountForPaymentType: j, getBonusAvailabilityForPaymentType: kt, getAvailableBonusesForPaymentType: M } = _e(), At = m(() => pe(wt), [wt]), { selectPaymentType: N, clearPaymentSelection: P } = ae(), { requestAccountDelete: jt, actionConfirmationModal: Mt } = Ce(), Nt = nt(!1), [Pt, Ft] = rt(null), [F, It] = rt({
		isComplete: !1,
		errorMessage: null,
		paymentTypeName: null
	}), [Lt, Rt] = rt(!1), [zt, Bt] = rt(!1), I = Dt.length > 0 ? Dt : Xe, L = ee(), Vt = we({
		uiComboView: b,
		lockAmount: dt,
		method: T
	}), Ht = v("cta.back"), { canGoBack: R, handleBack: Ut } = Ze({
		defaultRoute: Vt,
	...
	let Wt = b && !i(ft), z = p(() => {
		...
		if (!B || !S || !ct(B.fee)) return null;
		let t = st(B.fee, Et);
		if (!t) return null;
		let n = T === e.PAYOUT, r = {
			fee: t,
			method: v(n ? "cta.payout" : "cta.payin").toLocaleLowerCase(lt(gt))
	...
	if (Ot || I.length === 0) return /* @__PURE__ */ h($e, {
		text: "payment.no_pts_found",
		presentationScreen: "empty"
	});
	let In = /* @__PURE__ */ it("div", {
		className: fe(g.className("route-payment-types-content"), "px-1.5 @sm:px-3", O ? "pb-3 @md:pb-4" : "pb-4", ot),
		children: [
			/* @__PURE__ */ it("div", {
...
//#endregion
export { _ as PtsView, g as component };

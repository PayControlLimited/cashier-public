...
import { t as Ie } from "./_method-h5zacnZ5.js";
import { useCallback as f, useContext as Le, useEffect as p, useMemo as m, useRef as Re, useState as ze } from "react";
import { jsx as h, jsxs as Be } from "react/jsx-runtime";
//#region src/routes/pts/$method.tsx?tsr-split=component
var Ve = "pl-[0.9rem] pr-1.5 @sm:pl-[1.1rem] @sm:pr-3 @md:pl-[1.2rem] group-data-[stuck=true]:pb-0.5", He = "pt-2.5 @md:pt-3 [@container_(max-height:620px)]:pt-2.5 [@container_(max-height:520px)]:pt-2.25", Ue = (e, t) => e?.value ? e.feeType === "percentage" ? `${e.value}%` : t(e.value) || e.value : "", We = (e) => {
	if (!e?.value) return !1;
	...
	let r = e(t);
	return r === t ? n : r;
}, Ke = (e) => typeof e == "string" ? e : e.baseName;
function g() {
	let { paymentTypes: e } = Ie.useLoaderData();
	return /* @__PURE__ */ h(_, { loaderPaymentTypes: e });
}
function _({ loaderPaymentTypes: Ie = [] }) {
	let g = t(), { translateKey: _ } = me(), v = r(), { uiComboView: y, lockAmount: qe, uiComboView__PaymentTypes: b, uiInteractivePrompts: Je, uiPaymentConfirmView: Ye, uiShowFees: x, uiListStyle: Xe, selectedPaymentType: S, initialAmount: C, method: w, currency: T, user: E, locale: Ze, uiAccountDelete: Qe, uiListSelectable: $e, uiPreselectedPaymentType: et, onPaymentTypeSelected: D, paymentFormDraft: O, comboViewFormDraft: k, resetFlowOnBack: tt, setCashierState: A } = Le(ue), { currencyFormatter: nt, formatCurrencyValue: rt, formatDisplayValue: it } = Se(), { paymentTypesList: at, isError: j } = oe(), { getBonusCountForPaymentType: M } = n(), ot = m(() => de(nt), [nt]), { selectPaymentType: N, clearPaymentSelection: P } = i(), { requestAccountDelete: st, actionConfirmationModal: ct } = Ne(), lt = Re(!1), [ut, dt] = ze(null), [F, ft] = ze({
		isComplete: !1,
		errorMessage: null,
		paymentTypeName: null
	}), I = at.length > 0 ? at : Ie, pt = Oe({
		uiComboView: y,
		lockAmount: qe,
		method: w
		...
		if (!z || !x || !We(z.fee)) return null;
		let e = Ue(z.fee, it);
		if (!e) return null;
		let t = w === d.PAYOUT, n = {
			fee: e,
			method: Ge(_, t ? "cta.payout" : "cta.payin", t ? "Withdraw" : "Deposit").toLocaleLowerCase(Ke(Ze))
	...
	return ee("bottom", Pt), ge({
		...
		children: /* @__PURE__ */ Be("div", {
			className: ce(g.className("route-payment-types-content"), "px-1.5 @sm:px-3 pb-6 @md:pb-7", He),
